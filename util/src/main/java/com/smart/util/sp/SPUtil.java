...
public class SPUtil {
	 ...
	 * @param spName 缓存的名字
	 * @param keyStr 要存入缓存中的key
	 * @param valueStr 要存入缓存中的字符串数组
	 */
	public static void setStringSetIntoSp(String spName, String keyStr, Set<String> valueStr){
		SharedPreferences sp = mContext.getSharedPreferences(spName, MODE_PRIVATE);
		sp.edit().clear().putStringSet(keyStr, valueStr).commit();
	}
	...
	public static Set<String> getStringSetValueFromSP(String spName, String keyStr) {
		return getStringSetValueFromSP(spName, keyStr, null);
	}
	 ...
	 * @return 缓存中对应参数key的value
	 */
	public static Set<String> getStringSetValueFromSP(String spName, String keyStr, Set<String> defValues) {
		SharedPreferences sp = mContext.getSharedPreferences(spName, MODE_PRIVATE);
		return sp.getStringSet(keyStr, defValues);
	}
